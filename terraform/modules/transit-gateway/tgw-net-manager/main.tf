...
resource "null_resource" "aws_lz_ena_net_manager" {
    ...
    command = "aws networkmanager create-global-network --description '${var.global_network_description}' --tags '[Key=Name,Value=${var.global_network_name},${var.tags}]' > ${data.template_file.log_name.rendered}"
  }
}
