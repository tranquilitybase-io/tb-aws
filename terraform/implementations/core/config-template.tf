...
module "aws_lz_config_sns_topic" {
  ...
  required_tags = { (var.tag_key_project_id) = var.awslz_proj_id, (var.tag_key_environment) = var.awslz_environment, (var.tag_key_account_id) = local.log_archive_account_id, (var.tag_key_name) = "config" }
}
...
module "aws_lz_config_service" {
  ...
  config_logs_bucket = module.aws_lz_config_bucket.bucket_name_log
  s3_log_prefix = var.s3_log_prefix
  topic_account_id = local.log_archive_account_id
