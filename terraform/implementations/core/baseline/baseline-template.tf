...
module "aws_lz_config_service" {
  ...
  config_tags            = { (var.tag_key_project_id) = var.awslz_proj_id, (var.tag_key_environment) = var.awslz_environment, (var.tag_key_account_id) = sandbox_account_id, (var.tag_key_name) = "config" }
}
